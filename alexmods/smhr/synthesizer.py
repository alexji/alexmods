...
from . import (photospheres, radiative_transfer)
...
class Synthesizer(object):
    ...
    def __init__(self,twd=None,isotopes=None,R=None,verbose=False):
        ...
        self.twd = twd
        if verbose:
            print("Synthesizer working directory: {}".format(twd))
    ...
    def run_synth(self, atmosphere, linelist, abundances, smooth_fwhm=None, isotopes=None, verbose=False,
        ...
        out = out[0]
        spec = Spectrum1D(out[0],out[1],1e10*np.ones(len(out[0])))
        ...
        if self.R is not None or smooth_fwhm is not None:
            ...
            if verbose:
                print("Smoothing to FWHM={:.3f}A".format(smooth_fwhm))
